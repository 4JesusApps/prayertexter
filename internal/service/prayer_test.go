...
func (s *PrayerServiceSuite) TestComplete_RequesterNoLongerMember() {
	...
	s.members.EXPECT().Exists(s.ctx, "+19999999999").Return(false, nil)
...
func (s *PrayerServiceSuite) TestComplete_PrayerDeleteFails() {
	requestor := domain.Member{Phone: "+19999999999", Name: "Requestor"}
	intercessor := domain.Member{Phone: "+11234567890", Name: "Intercessor"}
	...
	s.prayers.EXPECT().Delete(s.ctx, "+11234567890", false).Return(errors.New("delete failed"))
...
func (s *PrayerServiceSuite) TestFindIntercessors_SkipsRequester() {
	s.intercessors.EXPECT().Get(s.ctx).Return(&domain.IntercessorPhones{
		Phones: []string{"+18888888888", "+19999999999"},
	}, nil)
	s.members.EXPECT().Get(s.ctx, "+19999999999").Return(&domain.Member{
	...
	result, err := s.svc.FindIntercessors(s.ctx, "+18888888888")
	s.Require().NoError(err)
	s.Len(result, 1)
	s.Equal("+19999999999", result[0].Phone)
}

func (s *PrayerServiceSuite) TestFindIntercessors_MixedActivePrayers() {
	s.intercessors.EXPECT().Get(s.ctx).Return(&domain.IntercessorPhones{
		Phones: []string{"+18888888888", "+19999999999"},
	}, nil)
	s.members.EXPECT().Get(s.ctx, "+18888888888").Return(&domain.Member{
...
func (s *PrayerServiceSuite) TestFindIntercessors_CapEnforcement() {
	s.intercessors.EXPECT().Get(s.ctx).Return(&domain.IntercessorPhones{
		Phones: []string{"+18888888888", "+19999999999", "+17777777777"},
	}, nil)
	s.members.EXPECT().Get(s.ctx, mock.Anything).Return(&domain.Member{
		Phone: "+18888888888", PrayerCount: 0, WeeklyPrayerLimit: 5,
	}, nil).Maybe()
	s.prayers.EXPECT().Exists(s.ctx, mock.Anything).Return(false, nil).Maybe()
...
func (s *PrayerServiceSuite) TestRequest_Success() {
	s.intercessors.EXPECT().Get(s.ctx).Return(&domain.IntercessorPhones{
		Phones: []string{"+18888888888", "+19999999999"},
	}, nil)
	s.members.EXPECT().Get(s.ctx, "+18888888888").Return(&domain.Member{
	...
	s.prayers.EXPECT().Save(s.ctx, mock.MatchedBy(func(p *domain.Prayer) bool {
		return p.Request == "please pray for my family and friends" &&
			p.Requestor.Name == "RealName"
	}), false).Return(nil).Times(2)
	introMsg, _ := messaging.Render(messaging.PrayerIntroTmpl, struct{ Name string }{"RealName"})
	expectedPrayerMsg := introMsg + "please pray for my family and friends" + "\n\n" + messaging.MsgPrayed
	s.sender.EXPECT().SendMessage(s.ctx, "+18888888888", expectedPrayerMsg).Return(nil)
	s.sender.EXPECT().SendMessage(s.ctx, "+19999999999", expectedPrayerMsg).Return(nil)
	s.sender.EXPECT().SendMessage(s.ctx, "+11234567890", messaging.MsgPrayerAssigned).Return(nil)
	...
	err := s.svc.Request(
		s.ctx,
		domain.TextMessage{Body: "please pray for my family and friends", Phone: "+11234567890"},
		mem,
	)
	s.NoError(err)
}

func (s *PrayerServiceSuite) TestRequest_Profanity() {
	...
	err := s.svc.Request(
		s.ctx,
		domain.TextMessage{Body: "please fuck pray for my family", Phone: "+11234567890"},
		mem,
	)
	s.NoError(err)
}

func (s *PrayerServiceSuite) TestRequest_AnonPrefix() {
	s.intercessors.EXPECT().Get(s.ctx).Return(&domain.IntercessorPhones{
		Phones: []string{"+18888888888", "+19999999999"},
	}, nil)
	s.members.EXPECT().Get(s.ctx, "+18888888888").Return(&domain.Member{
	...
	s.prayers.EXPECT().Save(s.ctx, mock.MatchedBy(func(p *domain.Prayer) bool {
		return p.Requestor.Name == "Anonymous" && !strings.Contains(strings.ToLower(p.Request), "#anon")
	}), false).Return(nil).Times(2)
	...
	err := s.svc.Request(
		s.ctx,
		domain.TextMessage{Body: "#anon please pray for my family and friends", Phone: "+11234567890"},
		mem,
	)
	s.NoError(err)
}

func (s *PrayerServiceSuite) TestRequest_AnonUppercaseAnywhere() {
	s.intercessors.EXPECT().Get(s.ctx).Return(&domain.IntercessorPhones{
		Phones: []string{"+18888888888", "+19999999999"},
	}, nil)
	s.members.EXPECT().Get(s.ctx, "+18888888888").Return(&domain.Member{
	...
	err := s.svc.Request(
		s.ctx,
		domain.TextMessage{Body: "please pray for #ANON my family and friends", Phone: "+11234567890"},
		mem,
	)
	s.NoError(err)
}

func (s *PrayerServiceSuite) TestRequest_PrayerSaveFailsRollsBack() {
	s.intercessors.EXPECT().Get(s.ctx).Return(&domain.IntercessorPhones{
		Phones: []string{"+18888888888"},
	}, nil)
	s.members.EXPECT().Get(s.ctx, "+18888888888").Return(&domain.Member{
		Phone: "+18888888888", Name: "I1", PrayerCount: 0, WeeklyPrayerLimit: 5,
	}, nil).Twice()
	...
	s.prayers.EXPECT().Save(s.ctx, mock.Anything, false).Return(errors.New("save failed"))
	s.members.EXPECT().Save(s.ctx, mock.MatchedBy(func(m *domain.Member) bool {
		return m.Phone == "+18888888888" && m.PrayerCount == 0
	})).Return(nil)
